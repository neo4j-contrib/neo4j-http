...
This PoC does best effort estimation on queries if they can be routed to readers or require writers to be executed. When in doubt, we will route things to a writer. It uses `EXPLAIN` per query to do this. This will happen only once per query, but is necessary so that the PoC works also without properly configured server side routing (SSR).
