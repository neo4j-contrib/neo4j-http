...
=== Rudimentary gRPC support

The streaming endpoint can be called with gRPC. With https://github.com/vadimi/grpc-client-cli[grpc-client-cli] this looks like this (Authentication is required):

```bash
echo '{"text": "MATCH (n:Movie) RETURN n.title"}' | \
  grpc-client-cli \
  --header "Authorization: Basic $(echo -n neo4j:secret | base64)" \
  --service Neo4jService \
  --method stream \
  localhost:9090
```

WARNING: This works only for properties in a nice way ATM and the messages are not yet fully defined.